...
export class WatchHelper {
  ...
  filterPath(entry) {
    ...
    if (stats && stats.isSymbolicLink()) {
      const result = this.filterDir(entry)

      chokidarDiag('[FILTER][SYMLINK]', {
    ...
    const isntIgnored = this.fsw._isntIgnored(resolvedPath, stats)
    const hasReadPermissions = this.fsw._hasReadPermissions(stats)

    const result = isntIgnored && hasReadPermissions

    if (!result) {
      chokidarDiag('[FILTER][REJECT]', {
        path: resolvedPath,
        isntIgnored,
        hasReadPermissions,
...
export class FSWatcher extends EventEmitter {
  ...
  async _emit(event, path, stats) {
    ...
    if (opts.alwaysStat && stats === undefined && (event === EV.ADD || event === EV.ADD_DIR || event === EV.CHANGE)) {
