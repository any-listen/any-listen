...
- 当输入为 `/changeLog` 且未提供有效正文时，则按本规范处理给定的文件。
- 当存在给定文件内容时，必须仅基于该内容生成最终双语 Markdown 结果，不输出过程说明。
- 当存在多个给定文件时，仅使用 **变更日志文件** 内容作为日志正文来源，其他文件仅作为规则参考，不得并入输出正文，不得更新 **变更日志文件** 以外的任何内容。
...
- 使用 Markdown 编写。
- 采用以下章节名称（按顺序）：
  - 英文：`### Added` / `### Improved` / `### Fixed` / `### Changed` / `### Deprecated` / `### Removed` / `### Security` / `### Other` / `### Extension System`
  - 中文：`### 新增` / `### 优化` / `### 修复` / `### 变更` / `### 弃用` / `### 移除` / `### 安全` / `### 其他` / `### 扩展系统`
...
- 若输入中出现裸引用（如 `#205`、`(#205)`），需在输出中规范化为标准 Markdown 链接格式。
- 语气保持客观、简洁、面向发布说明。
